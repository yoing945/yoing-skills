...
- [ ] **check-nav 模式检查**：只核验指向 `AGENTS.md` 的 guide 链接，leaf 条目不动；只核验/修补导航章节的不一致条目，保留现有名称与说明；严格沿文档引用递进，未发起任何不带具体路径的 Glob/Grep 调用；默认不写文件，仅 `--fix` 时写入；不为缺失目录新建 `AGENTS.md`
