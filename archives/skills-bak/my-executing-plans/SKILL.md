...
description: 当你有一份书面实现计划需要在当前会话中执行，并设有审查检查点时使用
---
...
**开始时宣布：** "我正在使用 my-executing-plans 技能来实现此计划。"
...
3. 实现 bug → 修复并重跑
4. 测试有问题 → 修复测试，向用户说明
5. 计划有误 → 停下来，向用户报告并建议修正

**依赖缺失：**
```
任务 3 需要 Redis 连接，但计划中没有提及 Redis 配置。
→ 停止执行
→ 向用户报告："任务 3 需要 Redis，计划中未包含配置步骤。
...
### 偏离计划的地方
- 任务 3：Redis 配置从 env 改为 config.yaml（经用户同意）
